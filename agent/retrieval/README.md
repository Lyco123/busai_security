...
The RAG retrieval service has already been brought online in the test environment and is now in the iteration phase. It is no longer just a clause CRUD prototype. The current implementation already includes:
...
Operationally, the service has already passed:

- retrieval API startup and health checks
- external ClickHouse deployment mode
- tunnel exposure through `cloudflared`
- preview upload recovery on an older external ClickHouse

The remaining work is iterative hardening, end-to-end validation, and permission/product polishing rather than first-time bring-up.

## API Endpoints

For the complete API contract, including request schemas, response schemas, storage semantics, permission inheritance, and error codes, see `agent/docs/current/operations/知识库系统API接口文档.md`.
...
- When `OCR_ENABLED=true`, call the bundled OCR service for scanned PDFs without a text layer
- Return `PDF_OCR_REQUIRED` for scanned PDFs when OCR is disabled
...
- Standard upload does not accept a client-supplied physical file path.
- Final raw files are stored under `RAW_FILE_ROOT/{kb_id}/{docId}/{timestamp}_{safeFileName}`.
- `POST /v1/documents` is a direct-create/backfill path that trusts `file_storage_key`; it does not upload or move raw files.
- `DELETE /v1/documents/{docId}` tombstones document and clauses and creates vector delete jobs, but it does not currently remove the raw physical file.
