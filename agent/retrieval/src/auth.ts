...
export function resolveCallerContext(request: FastifyRequest): CallerContext {
  const tenantId = readHeader(request, 'x-tenant-id') || DEFAULT_TENANT_ID;
  ...
  if (!callerLevelRaw || !isAccessLevel(callerLevelRaw)) {
    throw new ApiError(
      400,
      'INVALID_CALLER_LEVEL',
      'Header X-Caller-Level is required and must be driver/fleet/company/group'
    );
  }
  if (!callerId) {
