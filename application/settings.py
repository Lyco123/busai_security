...
EVENTS = [
    ...
    # "core.event.connect_clickhouse" if CLICKHOUSE_DB_ENABLE else None,
    # "core.event.multiprocessing_can_decrypt_event"
    # "core.event.multiprocessing_driver_event"
]
...
DB_CONFIG = {
    ...
    'password': 'jinqi2016'
    # 'schema': 'bus_ai',
    # 'option' : '-c search_path=dbo,public'
}
...
REPORT_START_TIME=22

#生成报告api地址
ai_api_url = "http://10.181.92.105:8001/ai/api/agent/reports/summary"

#提交报告地址
route_api_url= "http://10.181.92.105:8001/aisecurity-manager"

"""
线路画像参数配置
"""
route_profile_static_risk_weight = 0.7  # 车辆画像静态风险指标权重
route_profile_dynamic_risk_weight = 0.3  # 车辆画像动态风险指标权重

"""
事故黑点参数配置
"""
# 最小样本间隔,不同事故类型取值值不同，键表示事故类型（4种），值表示对应类型的最小样本间隔取值（需配置）
acc_min_meter = {1: 500,
                 2: 500,
                 3: 500,
                 4: 500}

# # 最小样本数,不同事故类型取值值不同，键表示事故类型（4种），值表示对应类型的最小样本数取值（需配置）
acc_min_spot = {1: 3,
                2: 3,
                3: 3,
                4: 3}

acc_distance_threshold = 500  # 黑点合并距离（单位：m）（需配置）
acc_behavior_coef = 50  # 等效事故权重
max_black_num_acc = 20  # 最大输出黑点数
max_reject_days_acc = 90  # 已拒绝黑点不再重复推荐的持续时间

"""
行为黑点参数配置
"""
# 最小样本间隔,不同行为类型取值值不同，键表示行为类型（34种），值表示对应类型的最小样本间隔取值（需配置）
behavior_min_meter = {
    ...
    33: 30, 34: 30, 36: 30, 37: 30}

# 最小样本数系数,键表示行为类型（34种），值表示对应行为类型系数。黑点最小样本数取值=类型样本总数x最小样本数系数（需配置）
min_spots_coef = {
    1: 0.300, 2: 0.250, 3: 0.400, 4: 0.250, 5: 0.250,
    6: 0.250, 7: 0.300, 8: 0.300, 9: 0.450, 10: 0.250,
    11: 0.250, 12: 0.250, 13: 0.250, 14: 0.580, 15: 0.480,
    16: 0.300, 17: 0.480, 18: 0.250, 19: 0.250, 20: 0.460,
    21: 0.430, 22: 0.520, 23: 0.250, 24: 0.300, 25: 0.250,
    26: 0.250, 27: 0.300, 28: 0.480, 29: 0.250, 30: 0.250,
    33: 0.580, 34: 0.250, 36: 0.650, 37: 0.600}

behavior_Min_Behavior_Sample = 10  # 行为黑点的最小违规行为次数（需配置）
behavior_distance_threshold = 30  # 黑点合并距离（单位：m）（需配置）
max_black_num_behavior = 30  # 最大输出黑点数
max_reject_days_behavior = 90  # 已拒绝黑点不再重复推荐的持续时间
