...
async def pre_process_mileage_data():
    ...
    # 重命名列
    mileage_by_driver.columns=['工号','driver_name','行驶总里程']
...
# ==================== 1. 数据加载与预处理 ====================
async def load_and_preprocess_data(datas):
    """加载数据并进行基础清洗"""
    data= pd.DataFrame(datas)
    data=data[1:]
    data.columns=['driver_name','性别','年龄','教育水平','驾龄','安全里程','日工时','历史事故次数',
                  ...
                  '心率','酒精浓度','收缩压','舒张压','脉搏','血氧','体温',
                  '心理测评等级','has_accident']

    mile_data=await pre_process_mileage_data()
    merged_data=data.merge(mile_data[['driver_name','行驶总里程']],on='driver_name',how='left')
    ...
    new_cols=['driver_name','性别','年龄','教育水平','驾龄','历史行驶总里程','历史安全行驶里程','日工时','历史事故率',
                  '违规使用N档','上坡不规范行为','下坡不规范行为','不文明鸣笛','不规范转弯',
                  '停站N档违规','停车不挂N档','全局超速','急减速','急加速','安全启动',
                  '区间超速','右转弯未刹车','左转弯未刹车','平路不规范行为','不规范开关门',
                  '急停','急刹车','不规范进站','熄火滑行','空档滑行','起步急加速',
    ...
    # 将 behavior_cols 列转换为 Decimal 类型
    # data[behavior_cols] = data[behavior_cols].applymap(Decimal)
    data[behavior_cols] = data[behavior_cols].map(Decimal)

    # 将 has_accident 转换为 Decimal 类型
    data['has_accident'] = data['has_accident'].apply(Decimal)
