...
async def pre_process_mileage_data():
    ...
    data = await read_raw_sql(
        "select ppartition,organ_name,driver_code as `工号` ,driver_name as `驾驶员`,total_mileage as `总里程` from ads_driver_mileage_yearly")
...
async def load_and_preprocess_data(data):
    ...
    new_cols=['司机名称','司机id','组织id','组织名称','性别','年龄','教育水平','驾龄','历史行驶总里程','历史安全行驶总里程','日工时','历史事故率',
              '违规使用N档','上坡不规范行为','下坡不规范行为','不文明鸣笛','不规范转弯',
              '停站N档违规','停车不挂N档','全局超速','急减速','急加速','安全启动',
              '区间超速','右转弯未刹车','左转弯未刹车','平路不规范行为','不规范开关门',
              '急停','急刹车','不规范进站','熄火滑行','空档滑行','起步急加速',
