...
# ==================== 10. 演示预测 ====================
async def prediction(start_time:str, end_time:str,calculation_type:str):
    """演示两层分数预测"""
    print("\n"+"="*90)
    print("评分系统")
    print("="*90)
    ...
    if calculation_type == '1':
        if datetime.now().strftime("%Y-%m-%d")==start_time:
            ...
            sql=driver_sql.predict_1d_sql(_data_date_str,_data_date_str)
        else:
            sql = driver_sql.predict_1d_sql(start_time, start_time)
    else:
        sql = driver_sql.predict_1d_sql(start_time,start_time)
    print(f"日sql:{sql}")
    ...
    #保存权重
    if calculation_type=="1":
        await save_accident_weight(results,start_time,end_time)
    else:
        await save_accident_score(results, start_time, end_time, info,base_cols, behavior_cols, illegal_cols)
    return results

if __name__=="__main__":
    # 分数预测
    results=asyncio.run(prediction('2026-07-10','2026-07-10',"1"))
