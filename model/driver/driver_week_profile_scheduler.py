...
def start_scheduler_sync_week_driver():
    """
    同步版本的调度器启动函数
    ...
    try:
        # 4. 添加任务
        scheduler.add_job(
            func=scheduled_task_driver_week_score,
            trigger=CronTrigger(day_of_week='mon', hour=HOUR, minute=MINUTE + 25),
            id='driver_score_week',
            name=f'驾驶员一周事故风险计算任务-每周一{HOUR}点{MINUTE+25}分',
        ...
        # 5. 启动调度器
        scheduler.start()
        logger.info("驾驶员一周事故风险计算任务调度器已在独立进程中启动")
...
def scheduled_task_driver_week_score():
    """
    驾驶员画像定时器 - 同步包装器
    ...
    try:
        print(f"{current_time}======{start_time}:计算驾驶员一周风险分数")

        result = asyncio.run(prediction_week(start_time,start_time,"0"))

        return result

    except Exception as e:
        # 修正日志描述，并打印堆栈跟踪以便调试
        logger.error(f"驾驶员一周风险分数计算任务失败: {e}", exc_info=True)
