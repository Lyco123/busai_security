...
@dataclass
class AbsAccidentQuotaScoreSub:
    """驾驶员画像指标分数子表实体类"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        """
        将AbsCompanyQuotaScoreSub对象转换为字典格式
...
class AbsAccidentSuggestedSub:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """
        将AbsCompanySuggestedSub对象转换为字典格式
...
class AbsAccidentProfileMain:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """
        将AbsCompanyProfileMain对象转换为字典格式
...
def main():
    # 示例用法
    print("单位画像系统实体类定义完成")
    print("- AbsCompanyProfileMain: 单位画像主表实体类（包含子类）")
    print("- AbsCompanyQuotaScoreSub: 单位画像指标分数子表实体类")
    print("- AbsCompanySuggestedSub: 单位画像建议子表实体类")

    # 创建示例数据
    profile = AbsAccidentProfileMain(
        ppartition="20260216",
        id="profile_001",
        Company_id="DRV001",
        Company_name="张三",
    ...
    print(f"\n创建示例单位画像: {profile.Company_name}")
    print(f"单位ID: {profile.company_id}")
